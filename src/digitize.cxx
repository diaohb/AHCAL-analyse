...
int noise_hitno[Layer_No][chip_No][channel_No];
int tau[Layer_No][chip_No][channel_No];
TH1I *h_sipm[15000];
...
int raw2Root::Digitize(string str_dat, string str_ped, string str_dac, string str_MIP,
        ...
        for (int i_chip = 0; i_chip < chip_No; ++i_chip) {
            ...
            for (int i_chan = 0; i_chan < channel_No; ++i_chan) {
                ...
                if (rms_low[i_layer][i_chip][i_chan] < 0)
                    rms_low[i_layer][i_chip][i_chan] = ref_rms_low;
    ...
    // read noise hitno
    fin = TFile::Open("/home/diaohb/CEPC/ihep/SPS/analyse/normal_list_noise.root", "READ");
    if (!fin) {
        cout << "cant open " << str_MIP << endl;
        return 0;
    }
    tree_in = (TTree *) fin->Get("noise");
    if (!tree_in) {
        cout << "cant get sipm gain tree" << endl;
        return 0;
    }
    fill(noise_hitno[0][0], noise_hitno[0][0] + 12960, 0);
    tree_in->SetBranchAddress("CellID", &CellID);
    tree_in->SetBranchAddress("Entries", &Entries);
    tree_in->SetBranchAddress("Tau", &Tau);
    for (int i = 0; i < tree_in->GetEntries(); ++i) {
        tree_in->GetEntry(i);
        decode_cellid(CellID, layer, chip, channel);
        tau[layer][chip][channel] = Tau;
        noise_hitno[layer][chip][channel] = Entries * exp(_MIP[layer][chip][channel] * 0.5 / Tau);
    }
    tree_in->Delete();
    fin->Close();
    // read dat
    ...
    for (int i = 0; i < tree_in->GetEntries(); ++i) {
        ...
        tree_in->GetEntry(i);
        // cout << i << endl;
        Digi_Event(hitTag, data_cellid, HG_Charge, LG_Charge, Hit_E, cellID);
...
int raw2Root::Digi_Event(vector<int> *hitTag, vector<int> *cellid, vector<double> *HG_Charge, vector<double> *LG_Charge, vector<double> *Hit_E, vector<int> *CellID) {
    double HG = 0, LG = 0;
    bool isnoise[40][9][36];
    fill(isnoise[0][0], isnoise[0][0] + 12960, 0);
    for (int layer = 0; layer < 40; layer++) {
        for (int chip = 0; chip < 9; chip++) {
            for (int channel = 0; channel < 36; channel++) {
                int cid = layer * 1e5 + chip * 1e4 + channel;
                double p = noise_hitno[layer][chip][channel] / 1842146.;
                // double p = 0;
                if (gRandom->Uniform() < p) {
                    isnoise[layer][chip][channel] = 1;
                    cellid->push_back(cid);
                    hitTag->push_back(1);
                    HG = gRandom->Exp(tau[layer][chip][channel]);
                    LG = HG / gain_ratio[layer][chip][channel];
                    HG_Charge->push_back(HG + ped_high[layer][chip][channel]);
                    LG_Charge->push_back(LG + ped_low[layer][chip][channel]);
                }
            }
        }
    }
    for (int i = 0; i < CellID->size(); i++) {
        ...
        int chip = cid / 10000 % 10;
        int channel = cid % 100;
        if (isnoise[layer][chip][channel] == 0) {
            hitTag->push_back(1);
            cellid->push_back(cid);
            digi(Hit_E->at(i), 0, HG, LG, cid);
            HG_Charge->push_back(HG);
            LG_Charge->push_back(LG);
        } else if (isnoise[layer][chip][channel] == 1 && gRandom->Uniform() < 0.5) {
            auto it = find(cellid->begin(), cellid->end(), cid);
            if (it != cellid->end()) {
                digi(Hit_E->at(i), 0, HG, LG, cid);
                HG_Charge->at(distance(cellid->begin(), it)) = HG;
                LG_Charge->at(distance(cellid->begin(), it)) = LG;
            }
        }
    }
    return 1;
